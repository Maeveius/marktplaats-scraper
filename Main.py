...
def scrape(search):
    ...
    while counter < hoeveel_int:
        ...
        for item in listings:
            ...
            website = item.find('a', attrs={'class': 'hz-Listing-sellerCoverLink'})
            mini_url = item.find('a', attrs={'class':'hz-Link'})

            title_text = title.get_text(strip=True)
            checker = title_garantes()
            title_value = checker.marktplaats_title(title_text, search)
            ...
            if title_value == True:
                ...
                dch = datum_check()
                datums = dch.marktplaats_datum(datum)
